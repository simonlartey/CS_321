...
public class EraserTool implements Tool, ResizableTool {
    private int lastX, lastY;
    private float eraserSize = 18f;

    @Override
    public void mouseDown(CanvasPanel canvas, MouseEvent e) {
        lastX = e.getX();
        lastY = e.getY();
        erase(canvas, lastX, lastY, lastX, lastY);
    }
    ...
    public void mouseDrag(CanvasPanel canvas, MouseEvent e) {
        int curX = e.getX();
        int curY = e.getY();
        erase(canvas, lastX, lastY, curX, curY);
        lastX = curX;
        lastY = curY;
    }
    ...
    private void erase(CanvasPanel canvas, int x1, int y1, int x2, int y2) {
